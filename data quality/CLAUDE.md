...
- Nothing here changes the data on its own — the checks only measure and
  report, and the one file that can lead to a change is changed by a
  person's own decision, read back from a file they saved themselves.
...
| File | What it does |
|---|---|
| `Compendium_Data_Quality.ipynb` | Part 1 reads the **raw questionnaires** and `external data\<Chapter>\`, writes `data_quality_review.txt` and one `Data quality issues before pipeline execution_<Chapter>.txt` per chapter, and gates the rest of the pipeline. Part 2 reads the **final long files**, writes `data_gaps_report.xlsx` and regenerates `docs/data.js` |
...
## Part 1 — before the pipeline runs, and the review-and-apply loop
...
- **`data_quality_review.txt`** (beside the per-chapter
  `pipeline_changes_<Chapter>.txt` / `need manual intervention_<Chapter>.txt`
  files) is the action file.
  - Every finding in sections 1 and 2 gets a `CORRECTION:` line, pre-filled
    with a suggestion where there is one.
  - A person edits the file directly — accept, retype, or `SKIP` — saves
    it, and tells Claude `apply <file>`.
  - `apply_review()` re-reads that exact file and writes the decisions to
    `translation dict.xlsx` (labels) and `value corrections.xlsx` (values,
    in `DATA COLLECTOR\`) — both idempotent and backed up first, the same
    pattern as notebook 4's `update_dictionary()`.
  - Nothing is written by any other path.
- **`Data quality issues before pipeline execution_<Chapter>.txt`**, one per
  chapter, is a separate, read-only summary — `write_chapter_reports()`,
  added 22 September 2026.
  - Not aggregated across chapters the way `data_quality_review.txt`'s
    findings are (the same mistyped header found in both Housing and
    Poverty gets its own line in each chapter's file, not one merged
    entry).
  - Never asks for a correction — `apply_review()` doesn't read it.
...
- Read-only.
  - If a change to the data is ever needed, it belongs in the pipeline
    project, not here — except Part 1's `apply_review()`, which changes
    exactly one thing (the dictionary and the value corrections file) and
    only on a person's own saved decision.
